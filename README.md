...
Inforamtion for deluxe, epic and ultra editions: In this 3 versions, we have the old and the value from the password. We don't want to display the passwords directly. For this "problem", we can use a similar way as we use it in the encryptables view. There, we display the password only if the user clicks on the text "show password". But we send the password together with the username from the backend to the frontend. For the logs, we would also send all attributes to the frontend and there, we would hide then the 2 values from the password.
